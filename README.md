...
<h4>2022 - 03 - 23 : </h4>
  - 진행 중에 재귀 쿼리를 이용하여 댓글 & 답댓글 방식으로 쿼리를 구현하는 도중 구현한 모델의 필드(컬럼)가 재귀 쿼리에서 생성된 정렬 컬럼과 같은 역활을할 수 있다는 것을 발견.<br>
  - 이번 작업을 통해 기획 단계에서 구채화가 모호한 점을 발견.<br>
  - 기획 단계로 다시 돌아가서 모호한 부분 및 구채적인 기획을 하는 것으로 변경.<br>
  - 요약<br>
  <ul>
    <li>재귀 쿼리로 Folder Side Navbar를 구현하려고 했지만 비슷한 역활을할 수 있는 필드 발견.</li>
    <li>이번 작업 진행으로 기획 단계에서 문제점을 발견함(구체적이지 않는 부분이 다수 존재)</li>
    <li>다시 기획 단계로 돌아가서 문제점 보안 하기.</li>
  </ul>

<h4>2022 - 03 - 18 ~ 22 </h4>
  - 17일날 발생된 문제 해결하느랴 여러 삽질함.<br>
  - 해결 방법으로 pymysql를 이용하여 쿼리를 정의하는(댓글&답 댓글 형식의 쿼리) 함수를 구현하기 위한 정보 수집 및 적용 작업 진행<br>
